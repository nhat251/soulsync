      ...
      <main className="flex-1 max-w-7xl mx-auto overflow-y-auto p-4 md:p-8">
            ...
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 text-text-muted dark:text-gray-400">
                    ...
                    <div className="flex items-center gap-2 font-semibold text-lg text-text-main dark:text-white">
                        ...
                        SoulSync <span className="text-text-muted dark:text-gray-500 font-normal">Insights</span>
                    </div>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="lg:col-span-2 bg-surface-light dark:bg-card-dark rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 relative overflow-hidden">
                <div className="flex justify-between items-start mb-6 z-10 relative">
                    <div>
                        <h2 className="text-xl font-bold mb-1 text-gray-800 dark:text-white">Xu hướng cảm xúc tuần này</h2>
                        <p className="text-sm text-text-muted dark:text-gray-400">Bạn đang có một tuần khá ổn định</p>
                    </div>
                    <div className="flex gap-4 text-xs font-medium">
                         <div className="flex items-center gap-1 text-primary">
                            <span className="w-2 h-2 rounded-full bg-primary"></span> Tích cực
                        </div>
                        <div className="flex items-center gap-1 text-text-muted dark:text-gray-400">
                             <span className="w-2 h-2 rounded-full bg-gray-300 dark:bg-gray-600"></span> Bình lặng
                        </div>
                    </div>
                </div>
                ...
                <div className="h-48 w-full mt-4">
                     <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={data}>
                            ...
                            />
                            <Area type="monotone" dataKey="value" stroke="#6B9080" strokeWidth={3} fillOpacity={1} fill="url(#colorValue)" />
                ...
                <div>
                    ...
                    <div className="bg-white/5 rounded-xl p-4 border border-white/10 backdrop-blur-sm mb-6">
                        <p className="text-sm leading-relaxed text-gray-300 italic">
                            "Bạn có xu hướng cảm thấy tích cực nhất vào buổi sáng sau khi tập thở. Sự lo âu thường xuất hiện vào chiều tối Thứ Năm."
                        </p>
                    </div>
                </div>
                 <div>
                    <h4 className="text-xs uppercase tracking-wider text-gray-500 mb-3 font-semibold">Chủ đề tuần này</h4>
                    <div className="flex flex-wrap gap-2">
                        {['Sự tập trung', 'Biết ơn', 'Sáng tạo'].map((tag, i) => (
                            <span key={i} className="px-3 py-1.5 rounded-full bg-white/10 text-xs font-medium border border-white/5 hover:bg-white/20 transition cursor-pointer">{tag}</span>
                        ))}
                    </div>
                </div>
            </div>
        ...
        <div className="bg-surface-light dark:bg-card-dark rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 mb-6">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h2 className="text-xl font-bold mb-1 text-gray-800 dark:text-white">Lịch sử cảm xúc tháng 10</h2>
                     <p className="text-sm text-text-muted dark:text-gray-400">Ghi nhận liên tiếp: <span className="font-semibold text-primary">12 ngày</span></p>
                </div>
            </div>
            
            <div className="grid grid-cols-7 gap-4 text-center">
                {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => <div key={d} className="text-xs text-text-muted dark:text-gray-500 uppercase mb-2">{d}</div>)}
                ...
                {[...Array(30)].map((_, i) => {
                    ...
                    return (
                        <div key={day} className={`aspect-square rounded-2xl flex flex-col items-center justify-center relative group hover:ring-2 hover:ring-primary/20 transition cursor-pointer text-sm font-medium
                        ...
                        `}>
                            {day}
                            {hasData && <span className={`w-1.5 h-1.5 rounded-full mt-1 ${opacity}`}></span>}
                        </div>
                    )
                })}
            </div>
            ...
            <div className="relative z-10 p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-primary text-white flex items-center justify-center shadow-lg shadow-primary/30 shrink-0">
                     ...
                     <div>
                        <div className="text-xs font-bold text-text-muted dark:text-gray-400 uppercase tracking-wider mb-1">Khuyến nghị của SoulSync</div>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-white">Dành riêng cho hôm nay</h3>
                        <p className="text-sm text-text-muted dark:text-gray-400 mt-1">Dựa trên cảm xúc hiện tại, hãy dành 5 phút để thư giãn hơi thở.</p>
                    </div>
                </div>
                 <Link to="/breathing" className="bg-primary hover:bg-primary-dark text-white px-6 py-3 rounded-xl font-medium shadow-lg shadow-primary/20 transition-all flex items-center gap-2 whitespace-nowrap group">
                    Bắt đầu ngay
